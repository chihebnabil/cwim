...
import { HealthStatus, DegradationRisk, } from '../../types/index.js';
export class CheckCommand {
    ...
    analyzeFiles(files, projectPath) {
    ...
    calculateHealth(analyses, projectPath) {
        ...
        // Risk level
        let riskLevel;
        if (hypotheticalUtilization > 0.9)
            riskLevel = DegradationRisk.CRITICAL;
        else if (hypotheticalUtilization > 0.7)
            riskLevel = DegradationRisk.HIGH;
        else if (hypotheticalUtilization > 0.5)
            riskLevel = DegradationRisk.MEDIUM;
        else
            riskLevel = DegradationRisk.NONE;
        // Concerns
        const concerns = [];
        if (largeFiles > 0)
            concerns.push(`${largeFiles} very large files (>10K tokens each)`);
        if (totalTokens > 100_000)
            concerns.push('Total codebase exceeds 100K tokens');
        if (fileCount > 500)
            concerns.push('Large number of source files');
        if (!claudeMdExists)
            concerns.push('No CLAUDE.md found');
        // Recommendations
