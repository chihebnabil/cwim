...
import { DegradationRisk, ClaudePlan, LogLevel, } from '../../types/index.js';
// ANSI escape codes
const CLEAR = '\x1Bc';
...
export class DashboardCommand {
    ...
    renderMiniBar(percent, width) {
        const filled = Math.max(0, Math.min(width, Math.floor(percent * width)));
        const empty = width - filled;
        const bar = '█'.repeat(filled) + '░'.repeat(empty);
        if (percent > 0.9)
            return this.theme.danger(bar);
        if (percent > 0.7)
            return this.theme.warning(bar);
        if (percent > 0.5)
            return this.theme.accent(bar);
