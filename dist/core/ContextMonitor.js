...
import { DegradationRisk, ContextCategory, AlertType, TrendDirection, SuggestionPriority, SuggestionCategory, } from '../types/index.js';
...
export class ContextMonitor extends EventEmitter {
    ...
    sessionStartTime;
    lastAlertLevel = DegradationRisk.NONE;
    consumptionHistory = [];
    ...
    updateFromContextCommand(model, usedTokens, totalWindow, breakdown, metadata) {
    ...
    updateManual(usedTokens, messageCount, fileReads, toolCalls, mcpServers, memoryFiles) {
        this.currentUsedTokens = usedTokens;
    ...
    addMessage(text, role) {
        ...
        // Update breakdown
        const messagesBreakdown = this.currentBreakdown.find(b => b.category === ContextCategory.MESSAGES);
    ...
    addMCPServer(serverName, toolCount) {
        const tokens = TokenEstimator.estimateMCPServer(serverName, toolCount);
        this.currentUsedTokens += tokens;
        this.currentMcpServers++;
        const mcpBreakdown = this.currentBreakdown.find(b => b.category === ContextCategory.MCP_TOOLS);
        if (mcpBreakdown) {
    ...
    removeMCPServer(serverName, toolCount) {
        const tokens = TokenEstimator.estimateMCPServer(serverName, toolCount);
    ...
    getBaselineTokens() {
        ...
        // Add memory files
        const memoryBreakdown = this.currentBreakdown.find(b => b.category === ContextCategory.MEMORY_FILES);
        if (memoryBreakdown) {
    ...
    calculateDegradationRisk(utilization) {
    ...
    estimateTurnsRemaining(freeTokens) {
        const rate = this.getConsumptionRate();
        if (rate.tokensPerMessage <= 0)
            return null;
    ...
    emitPredictionAlert(snapshot, prediction) {
        const alert = {
            id: uuidv4(),
            timestamp: new Date(),
    ...
    calculateTrend() {
        if (this.consumptionHistory.length < 5)
        ...
        if (avgSecondHalf < avgFirstHalf * 0.8)
            return TrendDirection.FALLING;
        return TrendDirection.STABLE;
    }
    /** Record consumption data point */
    recordConsumptionData(snapshot) {
